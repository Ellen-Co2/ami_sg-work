...
fd.fig
print (paste("mean error:",sum(abs(b_dfcast$mean-tst))/length(tst)))
print (paste("mean error rate:" ,sum(abs(b_dfcast$mean-tst)/tst)/length(tst)))
#tsdisplay(best_md.day$residuals)
```
#### change the time span and explore
```{r day&week with new period}
pt <- proc.time()
ms.tr <- msts(dat[(c(1:w.width)+leap),2],c(12,(12*24*7)))
tsdisplay(x = ms.tr)
